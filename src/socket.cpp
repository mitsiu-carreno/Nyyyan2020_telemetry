...
void sockethandler::ListenConnections(){
    ...
    server_address.sin_port = htons(8088);  // host byte order (unsigned short) -> network byte order  // Convert from little to big endian
    ...
    // Listening connections on address and port
    ...
    int bytes_in = recvfrom(sock_fd, buffer, 50, MSG_WAITALL, reinterpret_cast<struct sockaddr *>(&client_address), &client_length);
    if(bytes_in < 0){
      throw "No data received";
    }

    // echo "test" | nc -uw1 127.0.0.1 8088
    std::cout << "Msg: " <<  std::string(buffer) << "\n";
